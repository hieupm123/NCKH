program main
    ...
    integer*2, parameter :: NX = 24000, NY = 12000
    integer*2, dimension(NX, NY) :: data_all
    real*4, dimension(NX, NY) :: data_tbb
    real*4, dimension(5000) :: tbb
    integer*2, dimension(5000) :: cnt
    integer*2 :: i, j, index

    ! Nhận tham số đầu vào từ dòng lệnh
    call getarg(1, arg)
    call getarg(2, filelist)

    ! Mở file dữ liệu
    open(21, file=arg, access='direct', status='old', iostat=io, recl=NX*NY*2)
    if (io /= 0) then
        print *, "Lỗi mở file dữ liệu: ", trim(arg)
        stop
    end if

    read(21, rec=1, iostat=io) ((data_all(i, j), i=1, NX), j=1, NY)
    close(21)
    ...
    open(10, file="grid05.dat", access='direct', recl=NX*NY*4, status='replace', iostat=io)
    if (io /= 0) then
        print *, "Lỗi mở file đầu ra!"
        stop
    end if

    write(10, rec=1, iostat=io) ((data_tbb(i, j), i=1, NX), j=1, NY)
    close(10)
    ...
    print *, "Chuyển đổi hoàn tất! File đầu ra: grid05.dat"
end program
